...
plot_heatmap_bPCA_grb2_subs <- function(){
  
  # isolate grb2-sh3 data
  grb2_bind_df <- scaled_variants_bPCA[scaled_variants_bPCA$domain == "GRB2-SH3",]
  
  grb2_bind_subs_hm<-ggplot()+
    geom_tile(data=grb2_bind_df[grb2_bind_df$mut_type == "substitutions"], 
              aes(x=Pos, 
                  y=Mut,
                  fill=scaled_fitness))+
    ...
    ylab("grb2_bind")+
    geom_text(data=grb2_bind_df[grb2_bind_df$mut_type == "substitutions"], 
              aes(x=Pos,
                  y=Mut,
                  label=grb2_bind_df[grb2_bind_df$mut_type == "substitutions"]$significant))+
  ...
  grb2_seq_code <- grb2_bind_df[,c("Pos","Structure_name")]
  ...
  return(list(grb2_bind_subs_hm = grb2_bind_subs_hm,
