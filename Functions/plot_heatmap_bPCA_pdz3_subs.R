...
plot_heatmap_bPCA_pdz3_subs <- function(){
  
  # isolate PSD95-PDZ3 data
  pdz3_bind_df <- scaled_variants_bPCA[scaled_variants_bPCA$domain == "PSD95-PDZ3",]
  
  pdz3_bind_subs_hm<-ggplot()+
    geom_tile(data=pdz3_bind_df[pdz3_bind_df$mut_type == "substitutions"], 
              aes(x=Pos, 
                  y=Mut,
                  fill=scaled_fitness))+
    ...
    ylab("pdz3_bind")+
    geom_text(data=pdz3_bind_df[pdz3_bind_df$mut_type == "substitutions"], 
              aes(x=Pos,
                  y=Mut,
                  label=pdz3_bind_df[pdz3_bind_df$mut_type == "substitutions"]$significant))+
  ...
  pdz3_seq_code <- pdz3_bind_df[,c("Pos","Structure_name")]
  ...
  return(list(pdz3_bind_subs_hm = pdz3_bind_subs_hm,
