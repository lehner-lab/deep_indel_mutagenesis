plot_heatmap_bPCA_grb2_allins <- function(){
  
  grb2_bind_df <- scaled_variants_bPCA[scaled_variants_bPCA$domain == "GRB2-SH3",]
  
  grb2_bind_allins_hm<-ggplot()+
    geom_tile(data=grb2_bind_df[grb2_bind_df$type == "allins",], 
              aes(x=Pos, 
                  y=insID,
                  fill=scaled_fitness))+
    ...
    ylab("grb2_bind")+
    geom_text(data=grb2_bind_df[grb2_bind_df$type == "allins",], 
              aes(x=Pos,
                  y=insID,
                  label=grb2_bind_df[grb2_bind_df$type == "allins",]$significant))+
  ...
  return(list(grb2_bind_allins_hm = grb2_bind_allins_hm))
