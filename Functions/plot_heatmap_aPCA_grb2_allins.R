plot_heatmap_aPCA_grb2_allins <- function(){
  
  grb2_fold_df <- scaled_variants_aPCA[scaled_variants_aPCA$domain == "GRB2-SH3",]
  
  grb2_fold_allins_hm<-ggplot()+
    geom_tile(data=grb2_fold_df[grb2_fold_df$type == "allins",], 
              aes(x=Pos, 
                  y=insID,
                  fill=scaled_fitness))+
    ...
    ylab("grb2_fold")+
    geom_text(data=grb2_fold_df[grb2_fold_df$type == "allins",], 
              aes(x=Pos,
                  y=insID,
                  label=grb2_fold_df[grb2_fold_df$type == "allins",]$significant))+
    theme_gray()+
  ...
  return(list(grb2_fold_allins_hm = grb2_fold_allins_hm))
